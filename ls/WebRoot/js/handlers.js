...
var saveUrl;
//初始化 swf
function initSwfupload(saveurl,url,sizeLimit,types,typsdesc,uploadLimit){
		//初始化界面
		...
		var settings={
			...
			// Button Settings
			button_image_url : "images/select-files-zh_CN.png",
...
function init(uploadLimit,sizeLimit){
	...
	var uploadbody='<div id="uploadBody" class="ke-dialog-upload">'
		...
		+'		  <div class="ke-inline-block ke-swfupload-desc">可同时上传&nbsp;<font color="red">'+uploadLimit+'</font>&nbsp;张图片。单张图片容量不超过<font color="red">'+sizeLimit+'</font>，名称不超过<font color="red">30</font>汉字</div>'
		...
		+'		</div>'
		+'<input id="www"  type="button" class="ke-button-outer " value="确认">';
		+'	</div>'
		...
		/*+'	<span class="ke-button-common ke-button-outer" title="关闭"><input id="btncancel" class="ke-button-common ke-button" type="button" value="关闭"></span></div>'*/
...
$("#www").bind("click",function(){
	...
	for(var i=0;i<json.length;i++)
	{
		...
		//设置列内容和属性
		newTd0.innerHTML = json[i].id;
		var imghtml='';
			imghtml+="<img onmouseover='imgcorn(this)'style='cursor:pointer' data-toggle='modal'  data-target='#imgphoto' src='";
			imghtml+= json[i].path;
			imghtml+="'/>"
		newTd1.innerHTML= imghtml;	
		newTd2.innerHTML= json[i].name;
		
		var start = new Date();
			start.setTime(json[i].creation);
...
function insertAllevent(){
	...
	//向数据库中保存文件
	$.ajax({
	    url: saveUrl,    //请求的url地址
	    dataType: "text",   //返回格式为json
	    async: false, //请求是否异步，默认为异步，这也是ajax重要特性
	    data: {"urlList":urlList},    //参数值
	    type: "post",   //请求方式
	    beforeSend: function() {
	        //请求前的处理
	    },
...
function uploadSuccess(file, serverData) {
	try {
		...
		}else if(data.status == 5 || data.status == "5"){
			progress.setStatus("路径不对");
		}else if(data.status == 6 || data.status == "6"){
			progress.setStatus("创意名过长");
		}else if(data.status == 7 || data.status == "7"){
			progress.setStatus("创意名重复");
		}else{
			progress.setStatus("上传失败");
		}
